...
run() {
    log_info "Nmap — Fragment Scan"
    log_info "Splits packets into tiny fragments to evade packet-inspection firewalls (-f)."
    log_info "Useful when firewalls reassemble packets — fragmentation may slip through."
    echo ""
    ...
    log_step "Running: nmap -f $TARGET"
    echo ""
    nmap -f "$TARGET"

    echo ""
    log_success "Fragment scan complete."
}
