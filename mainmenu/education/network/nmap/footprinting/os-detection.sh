...
run() {
    log_info "Nmap — OS Detection"
    log_info "Attempts to identify the target's operating system (-O)."
    log_info "Analyzes TCP/IP stack behaviour for OS fingerprinting. Requires root."
    echo ""
    ...
    log_step "Running: nmap -O $TARGET"
    echo ""
    nmap -O "$TARGET"
