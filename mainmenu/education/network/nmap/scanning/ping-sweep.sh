...
run() {
    log_info "Nmap — Ping Sweep"
    log_info "Discovers live hosts on a network using ICMP ping (-sn)."
    log_info "No port scanning is performed — just host discovery."
    echo ""
    ...
    log_step "Running: nmap -sn $TARGET"
    echo ""
    nmap -sn "$TARGET"

    echo ""
    log_success "Ping sweep complete."
}
