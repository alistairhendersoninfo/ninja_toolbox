...
run() {
    log_info "Nmap — Stealth SYN Scan"
    log_info "Sends SYN packets without completing the TCP handshake (-sS)."
    log_info "Faster and stealthier than a full connect scan. Requires root."
    echo ""
    ...
    log_step "Running: nmap -sS $TARGET"
    echo ""
    nmap -sS "$TARGET"

    echo ""
    log_success "Stealth SYN scan complete."
}
