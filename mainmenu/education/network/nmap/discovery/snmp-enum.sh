...
run() {
    log_info "Nmap — SNMP Enumeration"
    log_info "Brute-forces SNMP community strings and gathers device info."
    log_info "Uses --script snmp-brute,snmp-info on port 161 (UDP)."
    echo ""
    ...
    log_step "Running: nmap --script snmp-brute,snmp-info -sU -p 161 $TARGET"
    echo ""
    nmap --script snmp-brute,snmp-info -sU -p 161 "$TARGET"

    echo ""
    log_success "SNMP enumeration complete."
}
